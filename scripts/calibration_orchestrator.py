...
def calculate_pose_diff(pose1_stamped: PoseStamped, pose2_stamped: PoseStamped):
    ...
    pos1 = np.array(
        [
            pose1_stamped.pose.position.x,
            pose1_stamped.pose.position.y,
            pose1_stamped.pose.position.z,
        ]
    )
    pos2 = np.array(
        [
            pose2_stamped.pose.position.x,
            pose2_stamped.pose.position.y,
    ...
    q1 = np.array(
        [
            pose1_stamped.pose.orientation.w,
            pose1_stamped.pose.orientation.x,
            pose1_stamped.pose.orientation.y,
            pose1_stamped.pose.orientation.z,
        ]
    )
    q2 = np.array(
        [
            pose2_stamped.pose.orientation.w,
            pose2_stamped.pose.orientation.x,
            pose2_stamped.pose.orientation.y,
            pose2_stamped.pose.orientation.z,
        ]
    )
...
class CalibrationOrchestrator(Node):
    def __init__(self):
        ...
        self.declare_parameter(
            "tracking_base_frame",
            f"{_TF_PREFIX}_color_optical_frame",  # Example default
            ParameterDescriptor(
                type=ParameterType.PARAMETER_STRING,
        ...
        self.declare_parameter(
            ...
            10.0,
            ParameterDescriptor(
                type=ParameterType.PARAMETER_DOUBLE, description="MoveIt planning time."
            ),
        )
        self.declare_parameter(
            "planning_attempts",
        ...
        self.post_move_delay_seconds = self.get_parameter(
            "post_move_delay_seconds"
        ).value
        ...
        self.take_sample_client = self.create_client(
            TakeSample, f"/{self.handeye_calibration_name}/take_sample"
        )
        self.compute_calibration_client = self.create_client(
            ComputeCalibration, f"/{self.handeye_calibration_name}/compute_calibration"
        )
        self.save_calibration_client = self.create_client(
            SaveCalibration, f"/{self.handeye_calibration_name}/save_calibration"
        )
        ...
        self.last_computed_calibration_transform: Transform = (
            None  # For tracking convergence
        )
        ...
        self.intermediate_calibrations_dir = (
            pathlib.Path.home() / ".ros/easy_handeye2/intermediate_calibrations"
        )
        try:
            os.makedirs(self.intermediate_calibrations_dir, exist_ok=True)
            self.get_logger().info(
                f"Intermediate calibrations will be saved to: {self.intermediate_calibrations_dir}"
            )

            # Initialize summary CSV file with headers
            self.summary_filepath = (
                self.intermediate_calibrations_dir
                / "calibration_convergence_summary.csv"
            )
            with open(self.summary_filepath, "w") as f_summary:
                f_summary.write(
                    "SampleNumber,TranslationDifference_m,RotationDifference_deg\n"
                )
            self.get_logger().info(
                f"Convergence summary will be saved to: {self.summary_filepath}"
            )

        except OSError as e:
            self.get_logger().error(
                f"Failed to create directory or summary file {self.intermediate_calibrations_dir}: {e}"
            )
    ...
    def _transform_to_pose_stamped(
        self, transform_msg: Transform, frame_id: str, stamp: Time
    ) -> PoseStamped:
    ...
    def _wait_for_robot_kinematic_chain_tf(self) -> bool:
        self.get_logger().info(
            "Waiting for robot kinematic chain TF (base to effector) to be available..."
        )
        ...
        if not rclpy.ok():
            self.get_logger().warn(
                "Shutdown requested while waiting for robot kinematic chain TF."
            )
            return False
    ...
    def _wait_for_tracking_tf_and_services(self) -> bool:
        self.get_logger().info(
            "Waiting for tracking system TF (tracking base to marker) to be available..."
        )
        ...
        while not tracking_frames_ok and rclpy.ok():
            try:
                ...
                if (
                    not rclpy.ok()
                ):  # Check if shutdown was requested before attempting to spin
                    self.get_logger().warn(
                        "Shutdown detected before spin_once in TF wait loop."
                    )
                ...
                try:
                    ...
                    self.get_logger().warn(
                        "Shutdown requested during spin_once in _wait_for_tracking_tf."
                    )
        ...
        self.get_logger().info(
            "Tracking TF confirmed. Now waiting for easy_handeye2 services..."
        )
        self.wait_for_services()

        if not rclpy.ok():  # Check if shutdown occurred during wait_for_services
            self.get_logger().warn(
                "Shutdown requested while waiting for easy_handeye2 services."
            )
            return False
    ...
    def _get_current_effector_pose_stamped(self) -> PoseStamped:
        ...
        filtered_joint_state.header = (
            full_joint_state.header
        )  # Retain header for timestamp consistency
        ...
        full_joint_positions_map = dict(
            zip(full_joint_state.name, full_joint_state.position)
        )
        ...
        for name in relevant_joint_names:
            if name in full_joint_positions_map:
                ...
                self.get_logger().warn(
                    f"Joint '{name}' from MoveIt2 joint_names not found in current full_joint_state. Skipping."
                )

        if len(filtered_joint_state.name) != len(relevant_joint_names):
            self.get_logger().error(
                "Mismatch in relevant joints found in full_joint_state. Cannot reliably compute FK."
            )
            return None
    ...
    def _check_sufficient_movement(self, current_pose_stamped: PoseStamped) -> bool:
        ...
        if (
            current_pose_stamped.header.frame_id
            != self.last_sampled_pose_stamped.header.frame_id
        ):
            self.get_logger().warn(
                f"Frame ID mismatch for pose comparison: "
        ...
        if (
            translation_diff >= self.min_translation_threshold_m
            or rotation_diff_rad >= self.min_rotation_threshold_rad
        ):
    ...
    def orchestrate_calibration_process(self):
        ...
        self.get_logger().info(
            "Performing initial setup: moving to the first calibration pose..."
        )
        ...
        for i, joint_state_rad in enumerate(self.target_joint_states_rad):
            ...
            if self.post_move_delay_seconds > 0:
                self.get_logger().info(
                    f"Pausing for {self.post_move_delay_seconds} seconds after movement for system stabilization..."
                )
                self.get_clock().sleep_for(
                    Duration(seconds=self.post_move_delay_seconds)
                )
            ...
            self.get_logger().debug(
                "Spinning once after potential delay and before sampling to update internal states."
            )
            ...
            if self._check_sufficient_movement(current_pose_stamped):
                ...
                take_sample_resp = self._call_service(
                    self.take_sample_client, take_sample_req, "TakeSample"
                )

                if (
                    take_sample_resp
                ):  # If TakeSample service call returned a response, assume success
                    ...
                    if (
                        ...
                        if compute_calib_resp:  # If ComputeCalibration service call returned a response, assume success
                            ...
                            current_calib_transform = (
                                compute_calib_resp.calibration.transform
                            )
                            ...
                            self.get_logger().info(
                                f"Computed Calibration Transform (sample {self.samples_taken}): "
                            ...
                            if self.last_computed_calibration_transform:
                                ...
                                trans_diff, rot_diff_rad = calculate_pose_diff(
                                    current_calib_pose, prev_calib_pose
                                )
                                ...
                                try:
                                    with open(self.summary_filepath, "a") as f_summary:
                                        f_summary.write(
                                            f"{self.samples_taken},{trans_diff:.6f},{rot_diff_deg:.4f}\n"
                                        )
                                except Exception as e_summary:
                                    self.get_logger().error(
                                        f"Failed to write to summary CSV: {e_summary}"
                                    )
                            else:
                                self.get_logger().info(
                                    "This is the first computed calibration, no previous to compare against."
                                )
                                # For the first calibration, we can record 0 difference or a special marker
                                try:
                                    with open(self.summary_filepath, "a") as f_summary:
                                        f_summary.write(
                                            f"{self.samples_taken},0.0,0.0\n"
                                        )  # Or N/A, N/A if preferred
                                except Exception as e_summary:
                                    self.get_logger().error(
                                        f"Failed to write initial entry to summary CSV: {e_summary}"
                                    )
                            ...
                            try:
                                intermediate_calib_filename = (
                                    f"calibration_sample_{self.samples_taken}.yaml"
                                )
                                intermediate_calib_filepath = (
                                    self.intermediate_calibrations_dir
                                    / intermediate_calib_filename
                                )
                                with open(intermediate_calib_filepath, "w") as f:
                                    f.write(
                                        message_to_yaml(compute_calib_resp.calibration)
                                    )
                                self.get_logger().info(
                                    f"Saved intermediate calibration to: {intermediate_calib_filepath}"
                                )
                            except Exception as e_save:
                                self.get_logger().error(
                                    f"Failed to save intermediate calibration: {e_save}"
                                )
                        else:
                            self.get_logger().warn(
        ...
        if self.samples_taken > 0:
            ...
            save_calib_resp = self._call_service(
                self.save_calibration_client, save_calib_req, "SaveCalibration"
            )
            if (
                save_calib_resp
            ):  # If SaveCalibration service call returned a response, assume success
...
def main(args=None):
    ...
    try:
        ...
        if orchestrator:
            ...
            print(
                f"CalibrationOrchestrator initialization failed: {e}"
        ...
        if orchestrator:
            orchestrator.get_logger().error(f"An unexpected error occurred: {e}")
        else:
            print(f"An unexpected error occurred in CalibrationOrchestrator: {e}")
    finally:
        if orchestrator:
